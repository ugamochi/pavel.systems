...
API_KEY="${N8N_API_KEY:-}"
WORKFLOW_FILE="${1:-n8n-workflows/stage-4-personalized.json}"
...
if [[ -z "$GSHEET_DOCUMENT_ID" || -z "$GSHEET_SHEET_NAME" || -z "$GSHEET_CREDENTIAL_NAME" ]]; then
  echo "Error: GSHEET_DOCUMENT_ID, GSHEET_SHEET_NAME, and GSHEET_CREDENTIAL_NAME are required for Stage 4." >&2
  exit 1
fi

if [[ -z "$GSHEET_CREDENTIAL_ID" || "$GSHEET_CREDENTIAL_ID" == "..." || "$GSHEET_CREDENTIAL_ID" == "{" ]]; then
  echo "Error: GSHEET_CREDENTIAL_ID is required for Stage 4 API deploy." >&2
  exit 1
fi
...
if [[ "$LLM_AUTH_MODE" == "bearer" && -z "$LLM_API_KEY" ]]; then
  cat >&2 <<'EOF'
Error: API key is required for Stage 4 lead scoring when LLM_AUTH_MODE=bearer.
